...
var URL ="/conferences/{conferenceid}/online";
...
let proxyId;
//查询在线会议信息API接口调用
export default class GetHistoryConfInfo extends React.Component {
     constructor () {
         super();
        ...
        this.state = {   
            ...
            type:typeCopy,
            queryType:queryTypeCopy
        }
     }
    
    handleOnClick = (e) => {
        const {pageIndex,pageSize,condition,type,queryType} = this.state;
                    ...
                    <Tabs defaultActiveKey='1'>
                        ...
                        <TabPane tab="Headers" key="2"> 
                            <div>
                                ...
                                <div style={{float:'left',height:'40px',width:'100%'}}><li><Input style={{width:'30%'}} readOnly value="type"/> : <Input style={{width:'60%'}} value={this.state.type} onChange={({target:{value}})=>{typeCopy=value; this.setState({type:value})}}/></li></div>
                                <div style={{float:'left',height:'40px',width:'100%'}}><li><Input style={{width:'30%'}} readOnly value="queryType"/> : <Input style={{width:'60%'}} value={this.state.queryType} onChange={({target:{value}})=>{queryTypeCopy=value; this.setState({queryType:value})}}/></li></div>
                                </ul>
                            </div>
                        </TabPane>
