...
var URL ="/historyConferences/{confuuid}/hisConfAttendeeRecord";
...
let server;
let proxyId;
//查询历史会议的与会者记录信息API接口调用
export default class GetHistoryConfAttendeeRecord extends React.Component {
     constructor () {
        super();
    ...
    handleOnClick = (e) => {
        const {pageIndex,pageSize,condition,language} = this.state;
        
        let params = {
            pageIndex,pageSize,condition,language
        }
                    ...
                    <Tabs defaultActiveKey='1'>
                        ...
                        <TabPane tab="Headers" key="2"> 
                            <div>
                                ...
                                <div style={{float:'left',height:'40px',width:'100%'}}><li><Input style={{width:'30%'}} readOnly value="language"/> : <Input style={{width:'60%'}} value={this.state.language} onChange={({target:{value}})=>{this.setState({language:value})}}/></li></div>
                                </ul>
                            </div>
                        </TabPane>
