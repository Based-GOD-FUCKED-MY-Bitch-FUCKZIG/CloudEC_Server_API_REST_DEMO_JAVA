...
export default class GetRecordFiles extends React.Component {
    ...
    handleOnClick = (e) => {
        const {accountID,pageIndex,pageSize,queryAll,condition} = this.state;
        
        let params = {
            accountID,pageIndex,pageSize,queryAll,condition
        }
        ...
        //查询在线会议列表
        get("/conferences/online", headers,params).then((response)=>
        {            
            ...
            <div style={{marginTop: '50px',marginLeft: '10%',width:'100%'}}>
                <div style={{width:'95%'}}>
                    ...
                    <div style={{float:'left',marginLeft: '20px',width:'60%'}}>
                        <Input addonBefore="URL" defaultValue="/conferences/online" readOnly/>
                    </div>
                </div> 
                <div style={{clear:'both'}}>
                    <Tabs defaultActiveKey='1'>
                        ...
                        <TabPane tab="Params" key="1">
                            <div>
                                <ul>
                                <div style={{float:'left',height:'40px',width:'100%'}}><li><Input style={{width:'30%'}} readOnly value="userUUID"/> : <Input style={{width:'60%'}} value={this.state.accountID} onChange={({target:{value}})=>{userUUIDCopy=value; this.setState({userUUID:value})}}/></li></div>
