...
import { ModifyCorpBody } from '@/component/custom/CommonConst'
...
let reqBody = JSON.stringify(ModifyCorpBody,null,4);
let resBody = '';
...
// let proxyId;
//修改企业API接口调用
export default class ModifyCorp extends React.Component {
    constructor () {
        super();
        this.state = {
            ...
            <div style={{marginTop: '50px',marginLeft: '10%',width:'100%'}}>
                <div style={{width:'95%'}}>
                    ...
                    <div style={{float:'left',marginLeft: '20px',width:'60%'}}>
                        <Input addonBefore="URL" value={this.state.url} onChange={({target:{value}})=>{URL=value;this.setState({url:value})}} style={{width:'500px'}}/>
                    </div>
                </div>
