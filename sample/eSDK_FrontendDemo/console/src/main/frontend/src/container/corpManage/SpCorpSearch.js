...
import { SpCorpSearchBody } from '@/component/custom/CommonConst'
...
let reqBody = JSON.stringify(SpCorpSearchBody,null,4);
let resBody = '';
...
// let proxyId;
//分页搜索企业API接口调用
export default class AddCorpDept extends React.Component {
    constructor () {
        super();
        this.state = {
    ...
    handleOnClick = () => {
      ...
      let data = reqBody;
      //分页搜索企业
      post("/sp/corp/search", {data}, headers).then(
        (res) => {
            ...
            <div style={{marginTop: '50px',marginLeft: '10%',width:'100%'}}>
                <div style={{width:'95%'}}>
                    ...
                    <div style={{float:'left',marginLeft: '20px',width:'60%'}}>
                        <Input addonBefore="URL" defaultValue="/rest/usg/datacenter/v1/sp/corp/search" style={{width:'500px'}} readOnly/>
                    </div>
                </div>
